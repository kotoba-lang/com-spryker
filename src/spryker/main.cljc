(ns spryker.main
  "Kotodama WASM entrypoint for the Spryker clean-room actor (L4) — Clojure port.
  ...
  facts (`spryker.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "spryker")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"spryker.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "spryker-compat" :tier tier :entities entities} 200])
...
(defn start! [] :spryker-compat/ready)
